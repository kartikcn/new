...
@Component({
  selector: 'app-view-room-report',
  templateUrl: './view-room-report.component.html',
  styleUrls: ['./view-room-report.component.scss'],
  providers: [MessageService]
})
export class ViewRoomReportComponent {
  ...
  loadRecords() {
    ...
    if (displayType == 'bar_chart') {
      this.blServ.getRoomReportsByFilter(data).subscribe((res: any) => {
        if (res) {
  ...
  getReportGridData() {
    ...
    this.blServ.getRoomReportsByFilterPaginated(data).subscribe((res: any) => {
      if (res) {
        this.isFiltered = false;
  ...
  printPDF() {
    ...
    this.bookingSrv.printPdf(reportDetails).subscribe((res: any) => {
      if (res != null) {
        this.messageService.add({ key: 'reportRoom', severity: 'success', summary: 'PDF Created', detail: 'PDF created successfully' });
  ...
  updateGridData(grid: string) {
    ...
    if (grid == 'Building' || grid == 'Division' || grid == 'Team' || grid == 'Room Category' || grid == 'Space Standard' || grid == 'Room Use') {
      this.showSecondParam = false;
      this.showThirdParam = false;
      this.gridRoomData = this.reportData.map((each: any) => ({ ...each, firstParamValue: each.name }));
    } else if (grid == 'Floor') {
      this.showSecondParam = true;
      this.showThirdParam = false;
      this.secondParamName = 'Building';
      this.gridRoomData = this.reportData.map((each: any) => (
      ...
      this.gridRoomData = this.reportData.map((each: any) => (
        { firstParamValue: each.rmType, secondParamValue: each.rmCat, ...each }
      ))
  ...
  onClickCount(data: any) {
    ...
    this.blServ.getRoomByFilter(filterdata).subscribe((res: any) => {
      this.roomData = res;
  ...
  getRmCatId(rmCat: any) {
    return this.chartReportData.find((each: any) => each.rmCat == rmCat)?.rmcatId;
  }
  getRmCatAndRmTypeId(rmCat: any, rmType: any) {
    let returnData = {
      rmcatId: null, rmtypeId: null
    }
  ...
  getSpaceStandard(ss: any) {
    return this.chartReportData.find((each: any) => each.spaceStandardName == ss)?.spaceStandard;
  }

  getRoomUse(ru: any) {
    return this.chartReportData.find((each: any) => each.rmUseName == ru)?.rmUse;
  }
