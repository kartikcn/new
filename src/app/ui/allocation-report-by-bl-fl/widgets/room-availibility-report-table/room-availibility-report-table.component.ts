...
@Component({
  selector: 'app-room-availibility-report-table',
  templateUrl: './room-availibility-report-table.component.html',
  styleUrls: ['./room-availibility-report-table.component.scss']
})
export class RoomAvailibilityReportTableComponent {
  @Input() data:any;
  rm_data:any=[];
  rm_array : any[]=[];
  ...
  exportExcel(){
    ...
    import("xlsx").then(xlsx => {
      const worksheet = xlsx.utils.json_to_sheet(this.rm_array);
        ...
        excelHeaders =  [ "Building Code","Floor Code","Room Code","Room Area (sq.ft)","Available Area (sq.ft)"]   
    ...
    getDestructuredData(data: any) {
      this.rm_array = data.map((item: any) => {
        return {
          ...
          "Room Area (sq.ft)":item['area'],
          "Available Area (sq.ft)":item['unallocatedArea']
        }
      });
    }
